...
public final class InvasionSkyRenderer {
	...
	public final boolean hasRenderedInvasionSky(final PoseStack pPoseStack, final Matrix4f pProjectionMatrix, final float pPartialTick, final Camera pCamera, final boolean pIsFoggy, final Runnable pSkyFogSetup) {
		pSkyFogSetup.run();
		...
		switch (this.skyType) {
		case NORMAL: {
			this.renderInvasionSky(level.getDayTime() % 12000L, pPoseStack, pProjectionMatrix, pPartialTick, pSkyFogSetup);
			return true;
		}
		case END: {
			this.renderEndInvasionSkybox(level.getDayTime() % 12000L, pPoseStack, pPartialTick);
			return true;
		}
	...
	private final void renderSun(final Matrix4f pMatrix4f, final float pSize, final long pDayTime) {
		...
		bufferBuilder.vertex(pMatrix4f, -pSize, 100.0F, -pSize).uv(0.0F, 0.0F).endVertex();
		bufferBuilder.vertex(pMatrix4f, pSize, 100.0F, -pSize).uv(1.0F, 0.0F).endVertex();
		bufferBuilder.vertex(pMatrix4f, pSize, 100.0F, pSize).uv(1.0F, 1.0F).endVertex();
		bufferBuilder.vertex(pMatrix4f, -pSize, 100.0F, pSize).uv(0.0F, 1.0F).endVertex();
		BufferUploader.drawWithShader(bufferBuilder.end());
	}
	...
	private final void renderMoon(final Matrix4f pMatrix4f, final float pSize, final long pDayTime) {
		...
		final BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
		bufferBuilder.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_TEX);
		bufferBuilder.vertex(pMatrix4f, -pSize, -100.0F, pSize).uv(f16, f17).endVertex();
		bufferBuilder.vertex(pMatrix4f, pSize, -100.0F, pSize).uv(f14, f17).endVertex();
		bufferBuilder.vertex(pMatrix4f, pSize, -100.0F, -pSize).uv(f14, f15).endVertex();
		bufferBuilder.vertex(pMatrix4f, -pSize, -100.0F, -pSize).uv(f16, f15).endVertex();
		BufferUploader.drawWithShader(bufferBuilder.end());
	}
